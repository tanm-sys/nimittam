...
/**
 * Process and sanitize LLM response output.
 * 
 * Handles:
 * - Newline escape sequences
 * - Special tokens that models may leak (<pad>, <eos>, etc.)
 * - Repetitive whitespace
 * - Common tokenizer artifacts
 */
fun processLlmResponse(response: String): String {
  // Special tokens that LLMs/tokenizers may leak into output
  // Covers: Gemma, GPT, BERT, T5, and other common model families
  val specialTokens = listOf(
    // Padding and sequence tokens
    "<pad>", "<PAD>", "<Pad>", "</s>", "<s>", "<eos>", "<bos>", "<unk>",
    // BERT-style tokens
    "[PAD]", "[CLS]", "[SEP]", "[UNK]", "[MASK]",
    // GPT/OpenAI style tokens
    "<|endoftext|>", "<|im_end|>", "<|im_start|>", "<|pad|>",
    // Gemma/Google specific
  ...
  // Remove pad token repetitions specifically (case-insensitive)
  cleaned = cleaned.replace(Regex("(<pad>\\s*)+", RegexOption.IGNORE_CASE), "")

  // Remove all special tokens (case-insensitive)
  for (token in specialTokens) {
    cleaned = cleaned.replace(token, "", ignoreCase = true)
  }
  
  // Remove XML-like tags (often hallucinated)
  cleaned = cleaned.replace(Regex("</?[a-zA-Z_]+>"), "")
  
  // Remove repeated special token patterns (e.g., multiple <pad> that might have spaces)
  cleaned = cleaned.replace(Regex("<[^>]{1,20}>"), "")
  
  // Clean up excessive whitespace (3+ spaces/newlines become single space)
